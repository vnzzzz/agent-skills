...
description: 非自明な新機能・仕様変更・バグ修正・リファクタリングの実装前に、解決する問題、既存実装、変更範囲、影響、検証方法、リスクを整理し、必要十分な変更計画を作るときに使用する。typoやrename等の明白で機械的な局所変更には使用しない。
---
...
1. **問題 / 期待結果** — 何を解決し、何が成立すれば完了か
2. **現在の実装** — 現在どこが責務を持ち、どう動いているか
3. **変更方針** — 採用する変更方針と、その理由
4. **変更範囲** — 直接変更する対象と波及対象
5. **非目標** — 今回変更しないもの
6. **検証** — 何をどう確認するか
7. **リスク / 未確認事項** — 重要な前提、未確認事項、追加調査
