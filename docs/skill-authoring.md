...
本文の言語とは分けて扱う。
H1は英語の識別名として扱うため、日本語やemojiを混在させない。技術名に必要な一般的なASCII記号は使用できるが、装飾目的の記号は使わない。
Title Case、connector、brand casing等の細かなstyleはレビューで確認する。CIはH1の存在、非空、英語識別名であることなど、壊れにくい構造条件だけを検証する。
